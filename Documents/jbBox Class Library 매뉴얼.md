...
| 창 제목 | OSC 0 / 2 파싱 → `set_title_cb`로 호스트에 전달 (그 외 OSC는 무시) |
...
자동 줄바꿈(`?7`)은 항상 켜져 있으며, 목록에 없는 DEC private 모드는 조용히 무시됩니다.
...
### 3.15 자식 프로세스에 전달되는 JBTERM 환경변수
...
이와 함께 `PROMPT` 환경변수도 자동으로 설정됩니다(SGR/커서 표시/마우스 리포팅 모드를 리셋하는 값). 두 값 모두, 자식 프로세스가 상속받은 환경변수 중 동일한 이름이 이미 있어도 덮어씁니다.
