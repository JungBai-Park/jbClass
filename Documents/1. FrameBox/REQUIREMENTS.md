...
- **Ctrl+Wheel zoom**: `FrameBox::PreTranslateMessage` intercepts `WM_MOUSEWHEEL` with `MK_CONTROL` before any child handles it. Each wheel detent snaps the on-screen scale -- `MulDiv(eff_dpi(), 100, 96)` percent, where `eff_dpi() = MulDiv(dpi, zoom_pm, 1000)` combines real monitor DPI with `zoom_pm` -- to the NEXT 5%-multiple strictly in the wheel direction (an arithmetic, not geometric, progression; a value already off the 5% grid snaps onto it on the first notch). `apply_zoom` clamps this percent to 25%~500% via `zoom_pm_for_pct()`, so `zoom_pm`'s own numeric bounds vary with the monitor's DPI. When the cursor is inside the frame, the pixel under the cursor is kept fixed on screen (cursor-anchor mode).
...
- `set_margin(int margin_96)`: when `margin_96 >= 0`, after every `rescale_children()` call `fit_to_children()` reads each WS_CHILD's actual rect (`GetWindowRect` + `ScreenToClient`) to find the maximum right/bottom edge, then sizes FrameBox's client area to fit all children plus `MulDiv(margin_96, eff_dpi(), 96)` physical pixels of padding (the client side scales with zoom). Default `-1` = disabled. Because `snap_to_grid()` inside ConBox runs synchronously within `MoveWindow`, `fit_to_children()` always sees the post-snap final child sizes.
- `fit_to_children()`'s non-client (caption/border) sizing is two-pass: pass 1 predicts it via `AdjustWindowRectExForDpi(..., dpi)` -- the real monitor DPI, not `eff_dpi()`, since Windows draws the caption/border at the monitor's own DPI regardless of `zoom_pm` -- and applies it with `SetWindowPos`; pass 2 then measures the window's actual `GetWindowRect() - GetClientRect()` and corrects with a second `SetWindowPos` only if pass 1 was off (no-op otherwise). See `PITFALLS.md` #20.
...
### 8 Host-Close Safety Net (WM_JBCLOSEQUERY)

- `WM_JBCLOSEQUERY = WM_APP + 101`: on `WM_CLOSE`, `FrameBox::WindowProc` sends this to every `WS_CHILD` registry entry before deciding whether to actually close. Same cross-module convention as `WM_JBZOOM` -- each module header (`FrameBox.h`, `ConBox.h`) defines the constant independently under an `#ifndef` guard with the same numeric value, so `FrameBox` and `ConBox` have no compile-time dependency on each other.
- A handler returns non-zero to mean "not ready yet". If ANY child returns non-zero, `FrameBox` calls `ShowWindow(SW_HIDE)` and returns 0 (swallowed) instead of falling through to the default `WM_CLOSE` handling (`DestroyWindow`). A child not ready to close is responsible for `PostMessage`-ing `WM_CLOSE` back to the frame once it actually is (see `ConBox::OnCloseQuery` / `handle_child_exit` in `Documents/2. ConBox/REQUIREMENTS.md`).
- Plain controls (buttons, edits, ...) have no handler for this message, so `DefWindowProc` returns 0 = ready by default; they never block a close. This makes the mechanism opt-in per child type with zero effect on children that do not care.
- The frame's own `WM_DESTROY` handling (clearing `event`/`WF_CONTINUEMODAL` for `wait()`) is unaffected: hiding via the guard does not touch that state, so a `Top.wait()` call already in `RunModalLoop()` simply keeps pumping messages (including the child's own retry timer) until the eventual real `DestroyWindow`.
